...
```{r process_and_summarize_results}
...
table(results_dffilt$sig)

p <- ggplot(data=results_dffilt, aes(x=beta, y=-log10(p), col=sig)) + 
    geom_point()
p
...
table(sig_intx$metabolite)[table(sig_intx$metabolite) %>% order(decreasing=T)] %>% head(10) %>% knitr::kable(caption = "Metabolites with most frequent significant relationships with microbes")
