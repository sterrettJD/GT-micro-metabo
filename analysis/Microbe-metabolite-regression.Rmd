...
degrees_g_all %>% 
    head(10) %>% 
    knitr::kable(caption="Microbes/metabolites with most FDR significant relationships", 
...
for (i in 1:10){
    ...
    feat_neighbors <- g_pos %>% neighbors(names(degrees_g_all[i]))
...
heatmap_df <- sig_intx %>% select(-V1, -annotated, -sig)

sig_intxing_mic_met <- c(sig_intx$microbe, sig_intx$metabolite)

for(mic in unique(sig_intx$microbe)){
    for(met in unique(sig_intx$metabolite)){
        # if it isn't already in heatmap_df
        if(nrow(filter(sig_intx,
                       microbe==mic, 
                       metabolite==met)) == 0){
            print(paste("finding", mic, met) )
            # get the corresponding row from edgelist
            new_row <- edgelist_df %>% 
                filter(feature_1 %in% c(mic, met),
                       feature_2 %in% c(mic, met))
            # map feature_1/2 to microbe/metabolite named column
            if(feature_1==mic){
                new_row <- new_row %>% 
                    rename(microbe = feature_1,
                           metabolite = feature_2)
            } else {
                new_row <- new_row %>% 
                    rename(metabolite = feature_1,
                           microbe = feature_2)
            }
            heatmap_df <- rbind(heatmap_df,
                                new_row)
        }
    }
}

heatmap_df %>% select(microbe, metabolite, beta) %>%
    pivot_wider(names_from = metabolite, values_from = beta)
...
rbind(results_lme_between, results_lme_within) %>% 
    filter(feature_1 %in% sig_intxing_mic_met,
           feature_2 %in% sig_intxing_mic_met) 


p <- ggplot(edgelist_df, aes(feature_1, feature_2, fill= beta)) + 
    geom_tile(colour="black", size=0.2) + 
    scale_x_discrete(labels = scales::label_wrap(10)) +
    coord_fixed(1) +
    theme_minimal()
