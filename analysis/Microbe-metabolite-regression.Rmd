...
```{r}
# create function to parallelize
run_lme_within <- function(i, j, scale_features=F){
    feature_1 <- feat_name_mapped[feat_name_mapped$old==i, 
                                "new"]
    feature_2 <- feat_name_mapped[feat_name_mapped$old==j, 
                                   "new"]
...
results_lme_genus <- data.frame(matrix(nrow=ncol(genus)^2,
                                       ncol=5))
...
y_lme <- foreach(i=1:ncol(genus),
    ...
    #                run_lme_within(feature_1, feature_2)
                    ...
                    tryCatch(run_lme_within(feature_1, feature_2),
                             error = function(e) 
                                return(list(e, NA, NA, NA, NA))
                    )
                }
            }
# names(metabo_df)[j]
...
edgelist_df <- results_dffilt_lme %>% 
    filter(p < 0.05) %>% 
    select(microbe, metabolite, beta)

adj_df <- edgelist_df %>% pivot_wider(values_from=beta, names_from=metabolite)

head(adj_df)


adj_mat <- adj_df %>% select(-microbe) %>% as.matrix()

dim(adj_mat)

graph_from_adjacency_matrix(adj_mat, mode='undirected', diag=F)
```
