...
TODO: MAP JANITOR AND PROCESSED NAMES BACK TO PREV VERSION
...
ggsave("../figures/LME-microbe-metabolite-volcano.png")
ggsave("../figures/LME-microbe-metabolite-volcano.pdf")
...
heatmap_df_long$plotmicrobe <- heatmap_df_long$microbe %>%
    sapply(function(x) x<- str_split(x, pattern="_")[[1]]) %>%
    sapply(function(x) ifelse(x[length(x)] %in% c("1855d", "ph2", "smb53"),
                              no=x[length(x)], 
                              yes=ifelse(x[length(x)]=="1855d",
                                         no=paste(x[length(x)-2],
                                                  x[length(x)]),
                                         yes=paste(x[length(x)-3],
                                                   x[length(x)-1],
                                                   x[length(x)]))
                              )
           )
...
# heatmap_df_long$plotmetabolite %>% unique() %>% as.data.frame() %>% write.table("metabo_name_mapper.txt", sep="\t")

# map the names to more visually friendly plotting names
metabo_name_mapper <- fread("metabo_name_mapper_for_use.csv")

name_hash <- hashmap()
sapply(metabo_name_mapper$Old, 
               function(x) {
    insert(name_hash, 
           x, 
           metabo_name_mapper[metabo_name_mapper$Old == x, New])
}
)

heatmap_df_long$plotmetabolite <- sapply(heatmap_df_long$plotmetabolite,
       function(x){
           gsub(x, #x=query(name_hash, x), 
                pattern="_", replacement=" ")
       }) 
# %>%
#        sapply(function(x) str_wrap(x, width=70))
