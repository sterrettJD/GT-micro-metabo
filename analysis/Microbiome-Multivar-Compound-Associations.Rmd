...
p <- EnhancedVolcano::EnhancedVolcano(toptable=adonis_df, x="lfc_hu_vs_gf", y="p",
                                 ...
                                 xlim=c(-10, 10), xlab=expression(Log[2](HU/GF)),
...
```{r protest_setup}
w_uni_pcoa <- ape::pcoa(no_pr_w_uni)
w_uni_pcoa_coords <- w_uni_pcoa$vectors
...
```{r run_protest}
protest(w_uni_pcoa_coords[,1:10], aqueous_bc_pcoa_coords[,1:10], permutations=9999)
protest(w_uni_pcoa_coords[,1:10], lipids_bc_pcoa_coords[,1:10], permutations=9999)


mantel(no_pr_w_uni, aqueous_bc)
mantel(no_pr_w_uni, lipids_bc)
```