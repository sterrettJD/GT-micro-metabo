...
check_dffits_clustered_feols <- function(estimate,
                                         df_threshold,
                                         data,
                                         compound, 
                                         covariates_formula=NULL,
...
for(compound in colnames(dplyr::select(all_faith_gt_cleannames, -c(faith_pd)))){
    ...
    dffits_passes <- check_dffits_clustered_feols(
...
faith_pd_plotdata <- all_faith_gt %>% 
    dplyr::select(c(feols_df[feols_df$padj < 0.05, "compound"],
                    faith_pd)) %>%
...
p <- ggplot(faith_pd_plotdata, 
    ...
    labs(x="Faith's phylogenetic diversity", y="Compound abundance")
p
```
